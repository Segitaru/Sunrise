...
void UOverloadWaveSpawnerComponent::Initialize(AOverloadLaneSpline* InLane, const TArray<FPawnFormation>& InFormations)
{
	Lane = InLane;
	CurrentFormations = InFormations;
	if (!GetOwner() || !GetOwner()->HasAuthority())
	{
		return;
	}
	if (!Lane.IsValid() || CurrentFormations.IsEmpty())
	{
		UE_LOG(LogTemp, Error, TEXT("Overload wave spawner %s cannot initialize: lane=%s definitions=%d"), *GetName(),
			*GetNameSafe(Lane.Get()), CurrentFormations.Num());
		return;
	}
...
{
	if (!GetOwner()->HasAuthority() || !Lane.IsValid() || CurrentFormations.IsEmpty())
	{
		return;
	}
	PruneTrackedUnits();
...
void UOverloadWaveSpawnerComponent::SpawnWaveForTeam(int32 TeamId, bool bSpawnAtSplineStart)
{
	...
	const FVector Forward = Spline->GetDirectionAtDistanceAlongSpline(SpawnDistance, ESplineCoordinateSpace::World);
	...
	int32 LineCount = CurrentFormations.Num();
	int32 SpawnedCount = 0;
	int32 ResultWaveSize = 0;
	for (const auto& Formation : CurrentFormations)
	{
		const UModularPawnData* Data = Formation.Definition.LoadSynchronous();
		...
		for (int32 Index = 0; Index < ResultCount; ++Index)
		{
			...
			if (!Unit)
			{
				UE_LOG(LogTemp, Warning, TEXT("Overload lane %s could not place definition %d for team %d without collision"),
					*GetNameSafe(Lane.Get()), Index % CurrentFormations.Num(), TeamId);
				continue;
			}
	...
	UE_LOG(LogTemp, Log, TEXT("Overload lane %s spawned %d/%d units for team %d at %s"), *GetNameSafe(Lane.Get()), SpawnedCount,
		ResultWaveSize, TeamId, bSpawnAtSplineStart ? TEXT("start") : TEXT("end"));
}
