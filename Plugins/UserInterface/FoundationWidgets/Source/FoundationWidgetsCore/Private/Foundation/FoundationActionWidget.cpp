...
#include "Foundation/FoundationActionWidget.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(FoundationActionWidget)

FSlateBrush UFoundationActionWidget::GetIcon() const
{
		...
		if (const UEnhancedInputLocalPlayerSubsystem* EnhancedInputSubsystem = GetEnhancedInputSubsystem())
		{
			...
			if (!BoundKeys.IsEmpty() && CommonInputSubsystem &&
				UCommonInputPlatformSettings::Get()->TryGetInputBrush(
					SlateBrush, BoundKeys[0], CommonInputSubsystem->GetCurrentInputType(), CommonInputSubsystem->GetCurrentGamepadName()))
			{
				return SlateBrush;
			}
		}
...
UEnhancedInputLocalPlayerSubsystem* UFoundationActionWidget::GetEnhancedInputSubsystem() const
{
