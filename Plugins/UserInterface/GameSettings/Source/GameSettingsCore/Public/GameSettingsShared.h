...
#include "GameSettingsShared.generated.h"
...
UENUM()
enum class EGameGamepadInputAPIOption : uint8
{
...
/**
 * UGameSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 */
UCLASS()
class GAMESETTINGSCORE_API UGameSettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(UGameSettingsShared, FOnSettingChangedEvent, UGameSettingsShared* Settings);
...
public:
	...
	UFUNCTION()
	static UGameSettingsShared* GetSharedSettings(ULocalPlayer* FromPlayer);
	...
	static UGameSettingsShared* CreateTemporarySettings(const ULocalPlayer* LocalPlayer);

	/** Synchronously loads a settings object, this is not valid to call before login */
	static UGameSettingsShared* LoadOrCreateSettings(const ULocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UGameSettingsShared* Settings);
...
private:
	...
	UPROPERTY()
	EGameGamepadInputAPIOption GamepadInputAPIOptions;

public:
	UFUNCTION()
	EGameGamepadInputAPIOption GetGamepadInputAPIOption() const { return GamepadInputAPIOptions; }
	UFUNCTION()
	void SetGamepadInputAPIOption(const EGameGamepadInputAPIOption NewValue);
