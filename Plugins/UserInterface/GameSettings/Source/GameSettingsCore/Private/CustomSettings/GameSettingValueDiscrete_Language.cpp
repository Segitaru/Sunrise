...
void UGameSettingValueDiscrete_Language::OnApply()
{
	...
	{
		Messaging->ShowConfirmation(UCommonGameDialogDescriptor::CreateConfirmationOk(LOCTEXT("WarningLanguage_Title", "Language Changed"),
...
{
	if (UGameSettingsShared* Settings = UGameSettingsShared::GetSharedSettings(LocalPlayer); IsValid(Settings))
	{
...
{
	if (const UGameSettingsShared* Settings = UGameSettingsShared::GetSharedSettings(LocalPlayer); IsValid(Settings))
	{
	...
	for (const FString& CultureName : AvailableCultureNames)
	{
		if (CultureName == TEXT(""))
		{
			...
			if (ensure(SystemDefaultCulture))
			{
				...
				FText LocalizedSystemDefault =
					FText::Format(LOCTEXT("SystemDefaultLanguage", "System Default ({0})"), FText::FromString(DefaultCultureDisplayName));
