...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GameAudioMixEffectsSubsystem)

class FSubsystemCollectionBase;

void UGameAudioMixEffectsSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	Super::Initialize(Collection);
}

void UGameAudioMixEffectsSubsystem::Deinitialize()
{
...
bool UGameAudioMixEffectsSubsystem::ShouldCreateSubsystem(UObject* Outer) const
{
...
void UGameAudioMixEffectsSubsystem::PostInitialize()
{
...
void UGameAudioMixEffectsSubsystem::OnWorldBeginPlay(UWorld& InWorld)
{
	...
	if (const UWorld* World = InWorld.GetWorld())
	{
		...
		if (const UGameSettingsLocal* GameSettingsLocal = GetDefault<UGameSettingsLocal>())
		{
			...
			if (UserMix)
			{
				...
				if (OverallControlBus && MusicControlBus && SoundFXControlBus && DialogueControlBus && VoiceChatControlBus)
				{
					const FSoundControlBusMixStage OverallControlBusMixStage =
						UAudioModulationStatics::CreateBusMixStage(World, OverallControlBus, GameSettingsLocal->GetOverallVolume());
					const FSoundControlBusMixStage MusicControlBusMixStage =
						UAudioModulationStatics::CreateBusMixStage(World, MusicControlBus, GameSettingsLocal->GetMusicVolume());
					const FSoundControlBusMixStage SoundFXControlBusMixStage =
						UAudioModulationStatics::CreateBusMixStage(World, SoundFXControlBus, GameSettingsLocal->GetSoundFXVolume());
					const FSoundControlBusMixStage DialogueControlBusMixStage =
						UAudioModulationStatics::CreateBusMixStage(World, DialogueControlBus, GameSettingsLocal->GetDialogueVolume());
					const FSoundControlBusMixStage VoiceChatControlBusMixStage =
						UAudioModulationStatics::CreateBusMixStage(World, VoiceChatControlBus, GameSettingsLocal->GetVoiceChatVolume());
...
void UGameAudioMixEffectsSubsystem::ApplyDynamicRangeEffectsChains(bool bHDRAudio)
{
...
void UGameAudioMixEffectsSubsystem::OnLoadingScreenStatusChanged(bool bShowingLoadingScreen)
{
	ApplyOrRemoveLoadingScreenMix(bShowingLoadingScreen);
}

void UGameAudioMixEffectsSubsystem::ApplyOrRemoveLoadingScreenMix(bool bWantsLoadingScreenMix)
{
...
bool UGameAudioMixEffectsSubsystem::DoesSupportWorldType(const EWorldType::Type World) const
{
