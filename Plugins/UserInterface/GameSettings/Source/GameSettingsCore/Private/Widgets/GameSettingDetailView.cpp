...
void UGameSettingDetailView::FillSettingDetails(UGameSetting* InSetting)
{
	...
	if (RichText_DisabledDetails)
	{
		...
		if (InSetting)
		{
			...
			{
				DisabledDetailLines.Add(LOCTEXT(
					"DisabledOptionReasonLine", "<Icon.Warning></> There are fewer options than available due to Parental Controls."));
			}
		}
		...
		if (InSetting)
		{
			...
			else
			{
				...
				StreamingHandle = UAssetManager::GetStreamableManager().RequestAsyncLoad(
					MoveTemp(ExtensionPaths), FStreamableDelegate::CreateWeakLambda(this,
												  [this, SettingPtr, ExtensionClassPtrs]
												  {
													  for (TSoftClassPtr<UGameSettingDetailExtension> SoftClassPtr : ExtensionClassPtrs)
													  {
