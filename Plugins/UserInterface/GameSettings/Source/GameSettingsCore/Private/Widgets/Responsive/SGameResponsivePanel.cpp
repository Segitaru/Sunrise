...
SGridPanel::FSlot& SGameResponsivePanel::AddSlot()
{
	...
	InnerGrid->AddSlot(InnerGrid->GetChildren()->Num(), 0).Expose(Slot);
	InnerSlots.Add(Slot);
...
void SGameResponsivePanel::OnArrangeChildren(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren) const
{
	ArrangedChildren.AddWidget(
		AllottedGeometry.MakeChild(ChildSlot.GetWidget(), FVector2D(0, 0), AllottedGeometry.GetLocalSize() / Scale, Scale));
}
