...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GamePlatformEmulationSettings)

#define LOCTEXT_NAMESPACE "GameCheats"

UGamePlatformEmulationSettings::UGamePlatformEmulationSettings()
{
}

FName UGamePlatformEmulationSettings::GetCategoryName() const
{
	return FApp::GetProjectName();
}

FName UGamePlatformEmulationSettings::GetPretendBaseDeviceProfile() const
{
	return PretendBaseDeviceProfile;
}

FName UGamePlatformEmulationSettings::GetPretendPlatformName() const
{
	return PretendPlatform;
}

#if WITH_EDITOR
void UGamePlatformEmulationSettings::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
...
void UGamePlatformEmulationSettings::PostReloadConfig(FProperty* PropertyThatWasLoaded)
{
...
void UGamePlatformEmulationSettings::PostInitProperties()
{
...
void UGamePlatformEmulationSettings::OnPlayInEditorStarted() const
{
	...
	{
		FNotificationInfo Info(FText::Format(LOCTEXT("PlatformTraitSuppressionActive", "Platform Trait Override\nSuppressing {0}"),
	...
	if (PretendPlatform != NAME_None)
	{
		FNotificationInfo Info(FText::Format(
			LOCTEXT("PlatformOverrideActive", "Platform Override Active\nPretending to be {0}"), FText::FromName(PretendPlatform)));
		Info.ExpireDuration = 3.0f;
		FSlateNotificationManager::Get().AddNotification(Info);
	}
}

void UGamePlatformEmulationSettings::ApplySettings()
{
...
void UGamePlatformEmulationSettings::ChangeActivePretendPlatform(FName NewPlatformName)
{
...
TArray<FName> UGamePlatformEmulationSettings::GetKnownPlatformIds() const
{
...
TArray<FName> UGamePlatformEmulationSettings::GetKnownDeviceProfiles() const
{
	TArray<FName> Results;
...
void UGamePlatformEmulationSettings::PickReasonableBaseDeviceProfile()
{
