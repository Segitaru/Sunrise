...
#include "Audio/GameAudioMixEffectsSubsystem.h"
#include "Audio/GameAudioSettings.h"
#include "AudioModulationStatics.h"
#include "CommonInputSubsystem.h"
#include "CommonUISettings.h"
#include "Development/GamePlatformEmulationSettings.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GameSettingsLocal)
...
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(
	TEXT("Game.Settings.ApplyFrameRateSettingsInPIE"), false, TEXT("Should we apply frame rate settings in PIE?"), ECVF_Default);
...
static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("Game.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false, TEXT("Do we apply front-end specific performance options in PIE?"), ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("Game.Settings.ApplyDeviceProfilesInPIE"), false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("Game.DeviceProfile.Console.TargetFPS"), -1, TEXT("Target FPS when being driven by device profile"), ECVF_Default | ECVF_Preview);

static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(TEXT("Game.DeviceProfile.Console.FrameSyncType"), -1,
	TEXT("Sync type when being driven by device profile. Corresponds to r.GTSyncType"), ECVF_Default | ECVF_Preview);

//////////////////////////////////////////////////////////////////////
// Mobile frame pacing

static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(TEXT("Game.DeviceProfile.Mobile.DefaultFrameRate"), 30,
	TEXT("Default FPS when being driven by device profile"), ECVF_Default | ECVF_Preview);

static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileMaxFrameRate(
	TEXT("Game.DeviceProfile.Mobile.MaxFrameRate"), 120, TEXT("Max FPS when being driven by device profile"), ECVF_Default | ECVF_Preview);

//////////////////////////////////////////////////////////////////////

static TAutoConsoleVariable<FString> CVarMobileQualityLimits(TEXT("Game.DeviceProfile.Mobile.OverallQualityLimits"), TEXT(""),
	TEXT(
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityLimits(TEXT("Game.DeviceProfile.Mobile.ResolutionQualityLimits"), TEXT(""),
	TEXT(
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityRecommendation(
	TEXT("Game.DeviceProfile.Mobile.ResolutionQualityRecommendation"), TEXT("0:75"),
	TEXT(
	...
	void UpdateCache()
	{
		...
		if (!CurrentValue.Equals(LastSeenCVarString, ESearchCase::CaseSensitive))
		{
			...
			while (ScanIndex < LastSeenCVarString.Len())
			{
				...
				if (ColonIndex > 0)
				{
					const int32 CommaIndex =
						LastSeenCVarString.Find(TEXT(","), ESearchCase::CaseSensitive, ESearchDir::FromStart, ColonIndex);
...
namespace GameSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FGameScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		...
		Mode.bHasOverrides |=
			UDeviceProfileManager::GetScalabilityCVar(FString::Printf(TEXT("sg.ShadowQuality%s"), *Suffix), Mode.Qualities.ShadowQuality);
		Mode.bHasOverrides |= UDeviceProfileManager::GetScalabilityCVar(
		...
		Mode.bHasOverrides |=
			UDeviceProfileManager::GetScalabilityCVar(FString::Printf(TEXT("sg.TextureQuality%s"), *Suffix), Mode.Qualities.TextureQuality);
		Mode.bHasOverrides |=
			UDeviceProfileManager::GetScalabilityCVar(FString::Printf(TEXT("sg.EffectsQuality%s"), *Suffix), Mode.Qualities.EffectsQuality);
		Mode.bHasOverrides |=
			UDeviceProfileManager::GetScalabilityCVar(FString::Printf(TEXT("sg.FoliageQuality%s"), *Suffix), Mode.Qualities.FoliageQuality);
		Mode.bHasOverrides |=
			UDeviceProfileManager::GetScalabilityCVar(FString::Printf(TEXT("sg.ShadingQuality%s"), *Suffix), Mode.Qualities.ShadingQuality);
	}
	...
	int32 ConstrainFrameRateToBeCompatibleWithOverallQuality(int32 FrameRate, int32 OverallQuality)
	{
		...
		int32 LimitIndex = PossibleRates.FindLastByPredicate(
			[=](const int32& TestRate)
			{
				...
				const bool bIsSupported = UGameSettingsLocal::IsSupportedMobileFramePace(TestRate);

				return bAtOrBelowDesiredRate && bQualityDoesntExceedLimit && bIsSupported;
			});

		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : UGameSettingsLocal::GetDefaultMobileFrameRate();
	}
...
void UGameSettingsLocal::SetToDefaults()
{
	...
	bEnableLatencyTrackingStats = UGameSettingsLocal::DoesPlatformSupportLatencyTrackingStats();
...
void UGameSettingsLocal::LoadSettings(bool bForceReload)
{
...
void UGameSettingsLocal::ResetToCurrentSettings()
{
...
void UGameSettingsLocal::BeginDestroy()
{
...
{
	return GEngine ? CastChecked<UGameSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void UGameSettingsLocal::ConfirmVideoMode()
{
...
float UGameSettingsLocal::GetEffectiveFrameRateLimit()
{
	const UGamePlatformSpecificRenderingSettings* PlatformSettings = UGamePlatformSpecificRenderingSettings::Get();
...
int32 UGameSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return GameSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void UGameSettingsLocal::OverrideQualityLevelsToScalabilityMode(
	const FGameScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");

	// Overrides any valid (non-negative) settings
	InOutLevels.ResolutionQuality =
		(InMode.Qualities.ResolutionQuality >= 0.f) ? InMode.Qualities.ResolutionQuality : InOutLevels.ResolutionQuality;
	InOutLevels.ViewDistanceQuality =
		(InMode.Qualities.ViewDistanceQuality >= 0) ? InMode.Qualities.ViewDistanceQuality : InOutLevels.ViewDistanceQuality;
	InOutLevels.AntiAliasingQuality =
		(InMode.Qualities.AntiAliasingQuality >= 0) ? InMode.Qualities.AntiAliasingQuality : InOutLevels.AntiAliasingQuality;
	InOutLevels.ShadowQuality = (InMode.Qualities.ShadowQuality >= 0) ? InMode.Qualities.ShadowQuality : InOutLevels.ShadowQuality;
	InOutLevels.GlobalIlluminationQuality = (InMode.Qualities.GlobalIlluminationQuality >= 0) ? InMode.Qualities.GlobalIlluminationQuality
...
void UGameSettingsLocal::ClampQualityLevelsToDeviceProfile(
	const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	...
	InOutLevels.ShadowQuality =
		(ClampLevels.ShadowQuality >= 0) ? FMath::Min(ClampLevels.ShadowQuality, InOutLevels.ShadowQuality) : InOutLevels.ShadowQuality;
	...
	InOutLevels.TextureQuality =
		(ClampLevels.TextureQuality >= 0) ? FMath::Min(ClampLevels.TextureQuality, InOutLevels.TextureQuality) : InOutLevels.TextureQuality;
	InOutLevels.EffectsQuality =
		(ClampLevels.EffectsQuality >= 0) ? FMath::Min(ClampLevels.EffectsQuality, InOutLevels.EffectsQuality) : InOutLevels.EffectsQuality;
	InOutLevels.FoliageQuality =
		(ClampLevels.FoliageQuality >= 0) ? FMath::Min(ClampLevels.FoliageQuality, InOutLevels.FoliageQuality) : InOutLevels.FoliageQuality;
	InOutLevels.ShadingQuality =
		(ClampLevels.ShadingQuality >= 0) ? FMath::Min(ClampLevels.ShadingQuality, InOutLevels.ShadingQuality) : InOutLevels.ShadingQuality;
}

void UGameSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UGameSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UGameSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void UGameSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool UGameSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
EGameStatDisplayMode UGameSettingsLocal::GetPerfStatDisplayState(EGameDisplayablePerformanceStat Stat) const
{
...
void UGameSettingsLocal::SetPerfStatDisplayState(EGameDisplayablePerformanceStat Stat, EGameStatDisplayMode DisplayMode)
{
...
bool UGameSettingsLocal::DoesPlatformSupportLatencyMarkers()
{
	return ICommonUIModule::GetSettings().GetPlatformTraits().HasTag(PerfStatTags::TAG_Platform_Trait_SupportsLatencyMarkers);
}

void UGameSettingsLocal::SetEnableLatencyFlashIndicators(const bool bNewVal)
{
...
void UGameSettingsLocal::SetEnableLatencyTrackingStats(const bool bNewVal)
{
...
void UGameSettingsLocal::ApplyLatencyTrackingStatSetting()
{
	...
	UE_CLOG(!LatencyMarkerModules.IsEmpty(), LogConsoleResponse, Log, TEXT("%s %d Latency Marker Module(s)"),
		bEnableLatencyTrackingStats ? TEXT("Enabled") : TEXT("Disabled"), LatencyMarkerModules.Num());
}

bool UGameSettingsLocal::DoesPlatformSupportLatencyTrackingStats()
{
	return ICommonUIModule::GetSettings().GetPlatformTraits().HasTag(PerfStatTags::TAG_Platform_Trait_SupportsLatencyStats);
}

float UGameSettingsLocal::GetDisplayGamma() const
{
	return DisplayGamma;
}

void UGameSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void UGameSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float UGameSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void UGameSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UGameSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void UGameSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UGameSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void UGameSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UGameSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void UGameSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void UGameSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
int32 UGameSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 UGameSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool UGameSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 UGameSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return GameSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 UGameSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return GameSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void UGameSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 UGameSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const UGamePlatformSpecificRenderingSettings* PlatformSettings = UGamePlatformSpecificRenderingSettings::Get();
...
void UGameSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const UGamePlatformSpecificRenderingSettings* PlatformSettings = UGamePlatformSpecificRenderingSettings::Get();
...
void UGameSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void UGameSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const UGamePlatformSpecificRenderingSettings* PlatformSettings = UGamePlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EGameFramePacingMode::MobileStyle)
	{
...
void UGameSettingsLocal::ClampMobileQuality()
{
	const UGamePlatformSpecificRenderingSettings* PlatformSettings = UGamePlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EGameFramePacingMode::MobileStyle)
	{
		...
		ensureMsgf(DefaultMobileFrameRate <= MaxMobileFrameRate,
			TEXT("Default mobile frame rate (%d) is higher than the maximum mobile frame rate (%d)!"), DefaultMobileFrameRate,
			MaxMobileFrameRate);
...
void UGameSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = GameSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
	...
	{
		UE_LOG(LogConsoleResponse, Log, TEXT("clamping mobile resolution quality max res: %f, %f, %f, %f, %f"), CurrentScaleNormalized,
			CurrentScaleValue, MinScaleValue, MaxScaleValue, MaxMobileResQuality);
		SetResolutionScaleValueEx(MaxMobileResQuality);
	}
}

void UGameSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = GameSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = GameSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
	...
	UE_LOG(
		LogConsoleResponse, Log, TEXT("Remap mobile resolution quality %f, %f, (%d,%d)"), CurrentScaleValue, ToResQuality, FromFPS, ToFPS);

	SetResolutionScaleValueEx(ToResQuality);
}

FString UGameSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void UGameSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

void UGameSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
	if (CanModifyHeadphoneModeEnabled())
	{
		static IConsoleVariable* BinauralSpatializationDisabledCVar =
			IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
...
bool UGameSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

bool UGameSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar =
		IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	const bool bHRTFOptionAvailable =
		BinauralSpatializationDisabledCVar && ((BinauralSpatializationDisabledCVar->GetFlags() & EConsoleVariableFlags::ECVF_SetByMask) <=
												  EConsoleVariableFlags::ECVF_SetByGameSetting);
...
bool UGameSettingsLocal::IsHDRAudioModeEnabled() const
{
	return bUseHDRAudioMode;
}

void UGameSettingsLocal::SetHDRAudioModeEnabled(bool bEnabled)
{
	...
	if (GEngine)
	{
		...
		{
			if (UGameAudioMixEffectsSubsystem* GameAudioMixEffectsSubsystem = World->GetSubsystem<UGameAudioMixEffectsSubsystem>())
			{
...
bool UGameSettingsLocal::CanRunAutoBenchmark() const
{
	const UGamePlatformSpecificRenderingSettings* PlatformSettings = UGamePlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool UGameSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void UGameSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
...
void UGameSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}

float UGameSettingsLocal::GetOverallVolume() const
{
	return OverallVolume;
}

void UGameSettingsLocal::SetOverallVolume(float InVolume)
{
...
float UGameSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void UGameSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UGameSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void UGameSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UGameSettingsLocal::GetDialogueVolume() const
{
	return DialogueVolume;
}

void UGameSettingsLocal::SetDialogueVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UGameSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void UGameSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
...
void UGameSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
...
void UGameSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}

void UGameSettingsLocal::ApplySafeZoneScale()
{
	SSafeZone::SetGlobalSafeZoneScale(GetSafeZone());
}

void UGameSettingsLocal::ApplyNonResolutionSettings()
{
...
int32 UGameSettingsLocal::GetOverallScalabilityLevel() const
{
...
void UGameSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == EGameFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit =
			GameSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void UGameSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName UGameSettingsLocal::GetControllerPlatform() const
{
...
void UGameSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void UGameSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
	...
	const FString EffectiveUserSuffix = UserFacingVariants.IsValidIndex(SuffixIndex) ? UserFacingVariants[SuffixIndex].DeviceProfileSuffix
	...
	if (GIsEditor)
	{
		const UGamePlatformEmulationSettings* Settings = GetDefault<UGamePlatformEmulationSettings>();
	...
	UE_LOG(LogConsoleResponse, Log,
		...
		PlatformMaxRefreshRate, *ExperienceSuffix, *UserChosenDeviceProfileSuffix, *EffectiveUserSuffix, *BasePlatformName,
...
void UGameSettingsLocal::UpdateConsoleFramePacing()
{
...
void UGameSettingsLocal::UpdateDesktopFramePacing()
{
...
void UGameSettingsLocal::UpdateMobileFramePacing()
{
...
void UGameSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
