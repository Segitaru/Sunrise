...
static TAutoConsoleVariable<bool> CVarShowDLCMenu(
	TEXT("Game.DLCMenu.Show"), false, TEXT("Whether to show the development-only DLC testing menu, if supported by the platform."));
...
void UGameSettingRegistry::AddDLCPage(UGameSettingCollection* Screen, ULocalPlayer* InLocalPlayer)
{
	...
	PlatformDLC->RegisterInitializationCallback(
		[WeakThis = TWeakObjectPtr<UGameSettingRegistry>(this), WeakDLCPage = TWeakObjectPtr<UGameSettingCollectionPage>(DLCPage),
			InitCondition, PlatformDLC]()
		{
			UGameSettingRegistry* This = WeakThis.Get();
			...
			for (const FName& DLCName : DLCNames)
			{
				...
				auto AddAction = [This, &Section, &DLCName, &AllActions](
									 FName DevName, FText Label, FText Desc, TFunction<void(ULocalPlayer*)> OnExecute)
				{
					...
					Action->SetDynamicDetails(FGetGameSettingsDetails::CreateWeakLambda(This,
						[DLCName](ULocalPlayer&) -> FText
						{
				...
				AddAction(FName(*FString::Printf(TEXT("DLC_%s_Download"), *DLCName.ToString())), LOCTEXT("DLC_Download", "Download"),
				...
				AddAction(FName(*FString::Printf(TEXT("DLC_%s_Mount"), *DLCName.ToString())), LOCTEXT("DLC_Mount", "Mount"),
				...
				AddAction(FName(*FString::Printf(TEXT("DLC_%s_Launch"), *DLCName.ToString())), LOCTEXT("DLC_Launch", "Launch"),
				...
				AddAction(FName(*FString::Printf(TEXT("DLC_%s_Unmount"), *DLCName.ToString())), LOCTEXT("DLC_Unmount", "Unmount"),
					...
					[DLCName](ULocalPlayer* LocalPlayer)
					{
						...
						if (World && World->GetMapName() == DLCName.ToString())
						{
							UCommonGameDialogDescriptor* Descriptor =
								UCommonGameDialogDescriptor::CreateConfirmationOkCancel(LOCTEXT("UnmountWarning_Header", "Unmount DLC?"),
									LOCTEXT("UnmountWarning_Body",
										"You are currently inside this DLC's map. Unmounting will likely crash the game. Are you sure?"));

							if (UCommonMessagingSubsystem* Messaging = LocalPlayer->GetSubsystem<UCommonMessagingSubsystem>())
							{
								Messaging->ShowConfirmation(Descriptor, FCommonMessagingResultDelegate::CreateWeakLambda(LocalPlayer,
																			[PlatformDLC, DLCName](ECommonMessagingResult Result)
																			{
				...
				AddAction(FName(*FString::Printf(TEXT("DLC_%s_Uninstall"), *DLCName.ToString())), LOCTEXT("DLC_Uninstall", "Uninstall"),
