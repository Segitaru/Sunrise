...
UGameSettingCollection* UGameSettingRegistry::InitializeGamepadSettings(ULocalPlayer* InLocalPlayer)
{
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			if (UCommonInputPlatformSettings* PlatformInputSettings =
					UPlatformSettingsManager::Get().GetSettingsForPlatform<UCommonInputPlatformSettings>())
			{
				...
				if (Setting->GetDynamicOptions().Num() > 1 && PlatformInputSettings->CanChangeGamepadType())
				{
					...
					const FName CurrentControllerPlatform = GetDefault<UGameSettingsLocal>()->GetControllerPlatform();
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UGameSettingsShared>()->GetForceFeedbackEnabled());
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UGameSettingsShared>()->GetInvertVerticalAxis());
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UGameSettingsShared>()->GetInvertHorizontalAxis());
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UGameSettingsShared>()->GetGamepadLookSensitivityPreset());

			for (int32 PresetIndex = 1; PresetIndex < (int32)EGameGamepadSensitivity::MAX; PresetIndex++)
			{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDescriptionRichText(
				LOCTEXT("LookSensitivityPresetAds_Description", "How quickly your view rotates while aiming down sights (ADS)."));
			...
			Setting->SetDefaultValue(GetDefault<UGameSettingsShared>()->GetGamepadTargetingSensitivityPreset());
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UGameSettingsShared>()->GetGamepadMoveStickDeadZone());
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UGameSettingsShared>()->GetGamepadLookStickDeadZone());
			Setting->SetDisplayFormat(UGameSettingValueScalarDynamic::ZeroToOnePercent);
			Setting->SetMinimumLimit(0.05);
			Setting->SetMaximumLimit(0.95);
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		// Enum option for which input API to use
		{
			...
			Setting->SetDescriptionRichText(LOCTEXT("GamepadInputAPI_Option_Description",
				"Which API should be used for processing gamepad input on PC devices.\n\nChanging this may improve compatibility with more input devices.\n\nChanging this setting requires a restart to take effect."));
			...
			Setting->SetDefaultValue(GetDefault<UGameSettingsShared>()->GetGamepadInputAPIOption());
			...
			Setting->AddEnumOption(EGameGamepadInputAPIOption::Legacy, LOCTEXT("GamepadAPI_Option_Legacy", "XInput + DualShock"));
			Setting->AddEnumOption(EGameGamepadInputAPIOption::Modern, LOCTEXT("GamepadAPI_Option_Modern", "Game Input"));
