...
DEFINE_LOG_CATEGORY_STATIC(GameMemoryDebugCommands, All, All)
...
FString WriteCollectionFile(const FString& CollectionName, const TArray<FString>& Items)
{
	...
	const FString OutputDir =
		WITH_EDITOR ? (FPaths::ProjectSavedDir() / TEXT("Collections")) : (FPaths::ProfilingDir() / TEXT("AssetSnapshots"));
...
FAutoConsoleCommandWithWorldAndArgs GObjListToCollectionCmd(TEXT("Lyra.ObjListToCollection"),
	...
	FConsoleCommandWithWorldAndArgsDelegate::CreateStatic(
		[](const TArray<FString>& Params, UWorld* World)
		{
			...
			const FString CollectionName = FString::Printf(
				TEXT("_LoadedAssets_%s_%s%s"), *FDateTime::Now().ToString(TEXT("%H%M%S")), *GWorld->GetMapName(), *CollectionNameSuffix);

			// Write the collection out
			const FString CollectionFilePath = WriteCollectionFile(CollectionName, AssetPaths);
			UE_LOG(GameMemoryDebugCommands, Warning, TEXT("Wrote collection of loaded assets to %s"), *CollectionFilePath);
		}));
...
void AnalyzeObjectListForDifferences(
	TArrayView<UObject*> ObjectList, UClass* CommonClass, const TSet<FName>& PropertiesToIgnore, bool bLogAllMatchedDefault = false)
{
	check(CommonClass);
	UObject* CommonClassCDO = CommonClass->GetDefaultObject();

	UE_LOG(GameMemoryDebugCommands, Log, TEXT("  Field\tDifferentToBase\tNumValues\tValues"));
	...
	for (TFieldIterator<FProperty> PropIt(CommonClass); PropIt; ++PropIt)
	{
		...
		if (!bAllMatchedDefaultValue)
		{
			...
			UE_LOG(GameMemoryDebugCommands, Log, TEXT("  %s::%s\t%s\t%d\t%s"), *CommonClass->GetName(), *Prop->GetName(),
				(ValuesObserved.Num() == 1) ? TEXT("FixedDifferent") : TEXT("Varies"), ValuesObserved.Num(), *ValueList);
		}
		else if (bLogAllMatchedDefault)
		{
			UE_LOG(GameMemoryDebugCommands, Log, TEXT("  %s::%s\t%s"), *CommonClass->GetName(), *Prop->GetName(), TEXT("Default"));
		}
	}
}
