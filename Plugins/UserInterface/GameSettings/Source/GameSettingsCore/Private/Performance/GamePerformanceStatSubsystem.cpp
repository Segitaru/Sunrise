...
#include "Performance/GamePerformanceStatSubsystem.h"
...
void FGamePerformanceStatCache::ProcessFrame(const FFrameData& FrameData)
{
	...
	{
		RecordStat(
			EGameDisplayablePerformanceStat::ClientFPS, (FrameData.TrueDeltaSeconds != 0.0) ? 1.0 / FrameData.TrueDeltaSeconds : 0.0);
	...
	if (UWorld* World = MySubsystem->GetGameInstance()->GetWorld())
	{
		...
		{
			RecordStat(EGameDisplayablePerformanceStat::ServerFPS, GAverageFPS);
		}
		...
		if (APlayerController* LocalPC = GEngine->GetFirstLocalPlayerController(World))
		{
			...
			if (UNetConnection* NetConnection = LocalPC->GetNetConnection())
			{
				...
				RecordStat(EGameDisplayablePerformanceStat::PacketSize_Incoming,
					(NetConnection->InPacketsPerSecond != 0) ? (NetConnection->InBytesPerSecond / (float)NetConnection->InPacketsPerSecond)
															 : 0.0f);
				RecordStat(EGameDisplayablePerformanceStat::PacketSize_Outgoing,
					(NetConnection->OutPacketsPerSecond != 0)
				...
				if (LatencyMarkerModule->GetEnabled())
				{
					...
					if (TotalLatencyMs > 0.0f)
					{
						...
						if (FCsvProfiler* Profiler = FCsvProfiler::Get())
						{
							...
							Profiler->RecordCustomStat(
								TotalLatencyStatName, CSV_CATEGORY_INDEX(GamePerformance), TotalLatencyMs, ECsvCustomStatOp::Set);

							static const FName GameLatencyStatName = TEXT("Game_Latency_Game");
							Profiler->RecordCustomStat(GameLatencyStatName, CSV_CATEGORY_INDEX(GamePerformance),
								LatencyMarkerModule->GetGameLatencyInMs(), ECsvCustomStatOp::Set);

							static const FName RenderLatencyStatName = TEXT("Game_Latency_Render");
							Profiler->RecordCustomStat(RenderLatencyStatName, CSV_CATEGORY_INDEX(GamePerformance),
								LatencyMarkerModule->GetRenderLatencyInMs(), ECsvCustomStatOp::Set);
						}
#endif
...
const FSampledStatCache* UGamePerformanceStatSubsystem::GetCachedStatData(const EGameDisplayablePerformanceStat Stat) const
{
