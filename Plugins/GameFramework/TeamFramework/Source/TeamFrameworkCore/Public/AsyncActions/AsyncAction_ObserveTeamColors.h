...
class ITFTeamAgentInterface;
class UTFTeamDisplayAsset;
struct FFrame;

DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(
	FTeamColorObservedAsyncDelegate, bool, bTeamSet, int32, TeamId, const UTFTeamDisplayAsset*, DisplayAsset);
...
public:
	...
	//  - For anything that can ever belong to a team (implements ITFTeamAgentInterface),
...
private:
	void BroadcastChange(int32 NewTeam, const UTFTeamDisplayAsset* DisplayAsset);

	UFUNCTION()
	void OnWatchedAgentChangedTeam(UObject* TeamAgent, int32 OldTeam, int32 NewTeam);

	UFUNCTION()
	void OnDisplayAssetChanged(const UTFTeamDisplayAsset* DisplayAsset);

	TWeakInterfacePtr<ITFTeamAgentInterface> TeamInterfacePtr;
