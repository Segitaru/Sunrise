...
#include "EFDeveloperSettings.generated.h"
...
class UEFDeveloperSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UEFDeveloperSettings();
...
public:
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category = EF, meta = (AllowedTypes = "EFExperienceDefinition"))
	FPrimaryAssetId ExperienceOverride;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category = EFBots, meta = (InlineEditConditionToggle))
	bool bOverrideBotCount = false;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category = EFBots, meta = (EditCondition = bOverrideBotCount))
	int32 OverrideNumPlayerBotsToSpawn = 0;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category = EFBots)
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category = EF)
	bool bTestFullGameFlowInPIE = false;

	/**
	* Should force feedback effects be played, even if the last input device was not a gamepad?
	* The default behavior in EF is to only play force feedback if the most recent input device was a gamepad.
	*/
	UPROPERTY(config, EditAnywhere, Category = EF)
	...
	UPROPERTY(config, EditAnywhere, Category = EF)
	TArray<FEFCheatToRun> CheatsToRun;
...
public:
	...
	EXPERIENCEFRAMEWORKCORE_API void OnPlayInEditorStarted() const;
