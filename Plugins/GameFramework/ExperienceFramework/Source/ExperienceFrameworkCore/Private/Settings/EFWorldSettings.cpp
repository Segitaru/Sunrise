...
#include UE_INLINE_GENERATED_CPP_BY_NAME(EFWorldSettings)

AEFWorldSettings::AEFWorldSettings(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

FPrimaryAssetId AEFWorldSettings::GetDefaultGameplayExperience() const
{
...
#if WITH_EDITOR
void AEFWorldSettings::CheckForErrors()
{
	...
	for (TActorIterator<APlayerStart> PlayerStartIt(GetWorld()); PlayerStartIt; ++PlayerStartIt)
	{
		...
		if (IsValid(PlayerStart) && PlayerStart->GetClass() == APlayerStart::StaticClass())
		{
			MapCheck.Warning()
				...
				->AddToken(FTextToken::Create(FText::FromString("is a normal APlayerStart, replace with AEFPlayerStart.")));
		}
	}
