...
#include UE_INLINE_GENERATED_CPP_BY_NAME(EFGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EEFEditorLoadMode
{
...
namespace EFGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(TEXT("EF.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via EFGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UEFGameplayCueManager::DumpGameplayCues));

	static EEFEditorLoadMode LoadMode = EEFEditorLoadMode::LoadUpfront;
} // namespace EFGameplayCueManagerCvars
...
UEFGameplayCueManager::UEFGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UEFGameplayCueManager* UEFGameplayCueManager::Get()
{
	return Cast<UEFGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UEFGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UEFGameplayCueManager::LoadAlwaysLoadedCues()
{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			else
			{
				UE_LOG(LogExperienceFramework, Warning,
					TEXT("UEFGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UEFGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (EFGameplayCueManagerCvars::LoadMode)
	{
		case EEFEditorLoadMode::LoadUpfront:
			return true;
		case EEFEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
			if (GIsEditor)
			{
				return false;
...
#endif
			break;
		case EEFEditorLoadMode::PreloadAsCuesAreReferenced:
			break;
	}
...
bool UEFGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UEFGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UEFGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UEFGameplayCueManager* GCM = Cast<UEFGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogExperienceFramework, Error, TEXT("DumpGameplayCues failed. No UEFGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogExperienceFramework, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogExperienceFramework, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogExperienceFramework, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogExperienceFramework, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogExperienceFramework, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogExperienceFramework, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
				...
				!GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogExperienceFramework, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogExperienceFramework, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogExperienceFramework, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogExperienceFramework, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogExperienceFramework, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogExperienceFramework, Log, TEXT("  ... %d cues in total"),
		GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UEFGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
	...
	if (bStartTask)
	{
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady(
			[]()
			{
				if (GIsRunning)
				{
					if (UEFGameplayCueManager* StrongThis = Get())
					{
...
void UEFGameplayCueManager::HandlePostGarbageCollect()
{
...
void UEFGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		else
		{
			UE_LOG(LogExperienceFramework, Warning,
				TEXT(
					"UEFGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UEFGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (EFGameplayCueManagerCvars::LoadMode)
	{
		case EEFEditorLoadMode::LoadUpfront:
...
void UEFGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UEFGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UEFGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UEFGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UEFGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UEFGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
