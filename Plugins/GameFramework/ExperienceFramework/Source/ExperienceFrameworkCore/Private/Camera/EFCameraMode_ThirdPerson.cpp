...
#include "Camera/EFCameraMode_ThirdPerson.h"

#include "Camera/EFCameraAssistInterface.h"
#include "Camera/EFCameraMode.h"
#include "Camera/EFPenetrationAvoidanceFeeler.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(EFCameraMode_ThirdPerson)

namespace EFCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UEFCameraMode_ThirdPerson::UEFCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FEFPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FEFPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FEFPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FEFPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FEFPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FEFPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FEFPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UEFCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UEFCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UEFCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UEFCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IEFCameraAssistInterface* TargetControllerAssist = Cast<IEFCameraAssistInterface>(TargetController);

	IEFCameraAssistInterface* TargetActorAssist = Cast<IEFCameraAssistInterface>(TargetActor);
	...
	IEFCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IEFCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IEFCameraAssistInterface* AssistArray[] = {TargetControllerAssist, TargetActorAssist, PPActorAssist};

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IEFCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UEFCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc,
	float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
	...
	//TODO IEFCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FEFPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(EFCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UEFCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UEFCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
