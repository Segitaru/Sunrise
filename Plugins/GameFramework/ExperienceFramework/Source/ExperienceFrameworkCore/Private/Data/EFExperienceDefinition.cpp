...
#include UE_INLINE_GENERATED_CPP_BY_NAME(EFExperienceDefinition)

#define LOCTEXT_NAMESPACE "EFSystem"

UEFExperienceDefinition::UEFExperienceDefinition()
{
}

#if WITH_EDITOR
EDataValidationResult UEFExperienceDefinition::IsDataValid(FDataValidationContext& Context) const
{
	...
	if (!GetClass()->IsNative())
	{
		...
		if (FirstNativeParent != ParentClass)
		{
			Context.AddError(FText::Format(
				...
				FText::AsCultureInvariant(GetPathNameSafe(ParentClass)), FText::AsCultureInvariant(GetPathNameSafe(FirstNativeParent))));
			Result = EDataValidationResult::Invalid;
		}
	}
...
void UEFExperienceDefinition::UpdateAssetBundleData()
{
