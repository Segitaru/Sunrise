...
#include "AsyncActions/AsyncAction_ObserveTeam.h"
...
UAsyncAction_ObserveTeam* UAsyncAction_ObserveTeam::InternalObserveTeamChanges(TScriptInterface<ISunriseTeamAgentInterface> TeamActor)
{
...
void UAsyncAction_ObserveTeam::SetReadyToDestroy()
{
	...
	if (ISunriseTeamAgentInterface* TeamInterface = TeamInterfacePtr.Get())
	{
...
void UAsyncAction_ObserveTeam::Activate()
{
	...
	if (ISunriseTeamAgentInterface* TeamInterface = TeamInterfacePtr.Get())
	{
		CurrentTeamIndex = SunriseTeamIdToInteger(TeamInterface->GetGenericTeamId());
