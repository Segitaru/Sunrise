...
class ISunriseTeamAgentInterface;
class USunriseTeamDisplayAsset;
struct FFrame;

DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(
	FTeamColorObservedAsyncDelegate, bool, bTeamSet, int32, TeamId, const USunriseTeamDisplayAsset*, DisplayAsset);
...
public:
	...
	//  - For anything that can ever belong to a team (implements ISunriseTeamAgentInterface),
...
private:
	void BroadcastChange(int32 NewTeam, const USunriseTeamDisplayAsset* DisplayAsset);

	UFUNCTION()
	void OnWatchedAgentChangedTeam(UObject* TeamAgent, int32 OldTeam, int32 NewTeam);

	UFUNCTION()
	void OnDisplayAssetChanged(const USunriseTeamDisplayAsset* DisplayAsset);

	TWeakInterfacePtr<ISunriseTeamAgentInterface> TeamInterfacePtr;
