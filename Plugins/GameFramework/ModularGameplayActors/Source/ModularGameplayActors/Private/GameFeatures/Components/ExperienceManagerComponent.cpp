...
#include "GameFeatures/ExperienceActionSet.h"
...
namespace ConsoleVariables
{
	...
	static FAutoConsoleVariableRef CVarExperienceLoadRandomDelayMin(TEXT(".chaos.ExperienceDelayLoad.MinSecs"),
		ExperienceLoadRandomDelayMin,
		TEXT(
			"This value (in seconds) will be added as a delay of load completion of the experience (along with the random value .chaos.ExperienceDelayLoad.RandomSecs)"),
		ECVF_Default);

	static float ExperienceLoadRandomDelayRange = 0.0f;
	static FAutoConsoleVariableRef CVarExperienceLoadRandomDelayRange(TEXT(".chaos.ExperienceDelayLoad.RandomSecs"),
		ExperienceLoadRandomDelayRange,
		TEXT(
			"A random amount of time between 0 and this value (in seconds) will be added as a delay of load completion of the experience (along with the fixed value .chaos.ExperienceDelayLoad.MinSecs)"),
		ECVF_Default);
...
void UExperienceManagerComponent::StartExperienceLoad()
{
	...
	UE_LOG(LogExperienceManagerComponent, Log, TEXT("EXPERIENCE: StartExperienceLoad(CurrentExperience = %s, %s)"),
...
void UExperienceManagerComponent::OnExperienceLoadComplete()
{
	...
	UE_LOG(LogExperienceManagerComponent, Log, TEXT("EXPERIENCE: OnExperienceLoadComplete(CurrentExperience = %s, %s)"),
		*CurrentExperience->GetPrimaryAssetId().ToString(), (HasAuthority() ? TEXT("Server") : TEXT("Client")));
...
void UExperienceManagerComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	if (LoadState == EExperienceLoadState::Loaded)
	{
		...
		{
			UE_LOG(LogExperienceManagerComponent, Error,
				TEXT("Actions that have asynchronous deactivation aren't fully supported yet in  experiences"));
		}
