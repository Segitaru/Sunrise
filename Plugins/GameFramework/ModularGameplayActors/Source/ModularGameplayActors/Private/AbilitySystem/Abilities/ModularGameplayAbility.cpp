...
#include "Abilities/ModularAbilityCost.h"
#include "Abilities/ModularAbilitySimpleFailureMessage.h"
...
UModularHeroComponent* UModularGameplayAbility::GetHeroComponentFromActorInfo() const
{
	return (CurrentActorInfo ? UModularHeroComponent::FindHeroComponent(CurrentActorInfo->AvatarActor.Get()) : nullptr);
}
...
bool UModularGameplayAbility::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	UModularAbilitySystemComponent* ModularASC = CastChecked<UModularAbilitySystemComponent>(ActorInfo->AbilitySystemComponent.Get());
	if (ModularASC->IsActivationGroupBlocked(ActivationGroup))
	{
		if (OptionalRelevantTags)
		{
			OptionalRelevantTags->AddTag(ModularGameplayTags::Ability_ActivateFail_ActivationGroup);
		}
		return false;
	}
...
void UModularGameplayAbility::SetCanBeCanceled(bool bCanBeCanceled)
{
	...
	{
		UE_LOG(LogModularAbilitySystem, Error, TEXT("SetCanBeCanceled: Ability [%s] can not block canceling because its activation group is replaceable."), *GetName());
		return;
	}
...
bool UModularGameplayAbility::CheckCost(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, OUT FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	for (const TObjectPtr<UModularAbilityCost>& AdditionalCost : AdditionalCosts)
	{
...
bool UModularGameplayAbility::DoesAbilitySatisfyTagRequirements(const UAbilitySystemComponent& AbilitySystemComponent, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, OUT FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	const UModularAbilitySystemComponent* ModularASC = Cast<UModularAbilitySystemComponent>(&AbilitySystemComponent);
	...
	{
#if UE_VERSION_5_8_x
		ModularASC->GetAdditionalActivationTagRequirements(GetAssetTags(), AllRequiredTags, AllBlockedTags);
#else
		ModularASC->GetAdditionalActivationTagRequirements(AbilityTags, AllRequiredTags, AllBlockedTags);
#endif
	}
	...
	if (AllBlockedTags.Num() || AllRequiredTags.Num())
	{
		...
		{
			if (OptionalRelevantTags && AbilitySystemComponentTags.HasTag(ModularGameplayTags::Status_Death))
			{
				// If player is dead and was rejected due to blocking tags, give that feedback
				OptionalRelevantTags->AddTag(ModularGameplayTags::Ability_ActivateFail_IsDead);
			}
...
bool UModularGameplayAbility::CanChangeActivationGroup(EModularAbilityActivationGroup NewGroup) const
{
	...
	UModularAbilitySystemComponent* ModularASC = GetModularAbilitySystemComponentFromActorInfo();
	check(ModularASC);

	if ((ActivationGroup != EModularAbilityActivationGroup::Exclusive_Blocking) && ModularASC->IsActivationGroupBlocked(NewGroup))
	{
...
bool UModularGameplayAbility::ChangeActivationGroup(EModularAbilityActivationGroup NewGroup)
{
	...
	if (ActivationGroup != NewGroup)
	{
		...
		ModularASC->AddAbilityToActivationGroup(NewGroup, this);
...
void UModularGameplayAbility::SetCameraMode(TSubclassOf<UModularCameraMode> CameraMode)
{
	ENSURE_ABILITY_IS_INSTANTIATED_OR_RETURN(SetCameraMode, );

	if (UModularHeroComponent* HeroComponent = GetHeroComponentFromActorInfo())
	{
