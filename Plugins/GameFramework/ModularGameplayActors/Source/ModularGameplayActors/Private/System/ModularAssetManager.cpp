...
const FName FModularBundles::Equipped("Equipped");

//////////////////////////////////////////////////////////////////////

static FAutoConsoleCommand CVarDumpLoadedAssets(
	TEXT("Modular.DumpLoadedAssets"),
...
UModularAssetManager& UModularAssetManager::Get()
{
	...
	UE_LOG(LogModularGameplayActors, Fatal, TEXT("Invalid AssetManagerClassName in DefaultEngine.ini.  It must be set to ModularAssetManager!"));
...
{
	UE_LOG(LogModularGameplayActors, Log, TEXT("========== Start Dumping Loaded Assets =========="));

	for (const UObject* LoadedAsset : Get().LoadedAssets)
	{
		UE_LOG(LogModularGameplayActors, Log, TEXT("  %s"), *GetNameSafe(LoadedAsset));
	}

	UE_LOG(LogModularGameplayActors, Log, TEXT("... %d assets in loaded pool"), Get().LoadedAssets.Num());
	UE_LOG(LogModularGameplayActors, Log, TEXT("========== Finish Dumping Loaded Assets =========="));
}
...
void UModularAssetManager::InitializeGameplayCueManager()
{
	SCOPED_BOOT_TIMING("UModularAssetManager::InitializeGameplayCueManager");

	UModularGameplayCueManager* GCM = UModularGameplayCueManager::Get();
	check(GCM);
	GCM->LoadAlwaysLoadedCues();
}


const UModularGameData& UModularAssetManager::GetGameData()
{
	return GetOrLoadTypedGameData<UModularGameData>(ModularGameDataPath);
}

const UModularPawnData* UModularAssetManager::GetDefaultPawnData() const
{
...
UPrimaryDataAsset* UModularAssetManager::LoadGameDataOfClass(TSubclassOf<UPrimaryDataAsset> DataClass, const TSoftObjectPtr<UPrimaryDataAsset>& DataClassPath, FPrimaryAssetType PrimaryAssetType)
{
	...
	{
#if WITH_EDITOR
		FScopedSlowTask SlowTask(0, FText::Format(NSLOCTEXT("ModularEditor", "BeginLoadingGameDataTask", "Loading GameData {0}"), FText::FromName(DataClass->GetFName())));
		const bool bShowCancelButton = false;
		const bool bAllowInPIE = true;
		SlowTask.MakeDialog(bShowCancelButton, bAllowInPIE);
#endif
		UE_LOG(LogModularGameplayActors, Log, TEXT("Loading GameData: %s ..."), *DataClassPath.ToString());
	...
	else
	{
		...
		UE_LOG(LogModularGameplayActors, Fatal, TEXT("Failed to load GameData asset at %s. Type %s. This is not recoverable and likely means you do not have the correct data to run %s."), *DataClassPath.ToString(), *PrimaryAssetType.ToString(), FApp::GetProjectName());
	}
...
void UModularAssetManager::DoAllStartupJobs()
{
	...
	UE_LOG(LogModularGameplayActors, Display, TEXT("All startup jobs took %.2f seconds to complete"), FPlatformTime::Seconds() - AllStartupJobsStartTime);
}
...
#if WITH_EDITOR
void UModularAssetManager::PreBeginPIE(bool bStartSimulate)
{
	Super::PreBeginPIE(bStartSimulate);

	{
		FScopedSlowTask SlowTask(0, NSLOCTEXT("ModularEditor", "BeginLoadingPIEData", "Loading PIE Data"));
		const bool bShowCancelButton = false;
		const bool bAllowInPIE = true;
		SlowTask.MakeDialog(bShowCancelButton, bAllowInPIE);

		const UModularGameData& LocalGameDataCommon = GetGameData();
