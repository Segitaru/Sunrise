...
#include "System/ModularDevelopmentStatics.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ModularDevelopmentStatics)

//@TODO: Actually want to take a lambda and run on every authority world most of the time...
UWorld* UModularDevelopmentStatics::FindPlayInEditorAuthorityWorld()
{
	check(GEngine);
...
TArray<FAssetData> UModularDevelopmentStatics::GetAllBlueprints()
{
...
UClass* UModularDevelopmentStatics::FindBlueprintClass(const FString& TargetNameRaw, UClass* DesiredBaseClass)
{
	FString TargetName = TargetNameRaw;
	TargetName.RemoveFromEnd(TEXT("_C"), ESearchCase::CaseSensitive);

	TArray<FAssetData> BlueprintList = UModularDevelopmentStatics::GetAllBlueprints();
...
UClass* UModularDevelopmentStatics::FindClassByShortName(const FString& SearchToken, UClass* DesiredBaseClass, bool bLogFailures)
{
