...
//////////////////////////////////////////////////////////////////////////
FModularCameraModeView::FModularCameraModeView()
	: Location(ForceInit)
	, Rotation(ForceInit)
	, ControlRotation(ForceInit)
	, FieldOfView(MODULAR_CAMERA_DEFAULT_FOV)
{
}

void FModularCameraModeView::Blend(const FModularCameraModeView& Other, float OtherWeight)
{
	if (OtherWeight <= 0.0f)
	{
		return;
...
// UModularCameraMode
//////////////////////////////////////////////////////////////////////////
UModularCameraMode::UModularCameraMode()
{
	FieldOfView = MODULAR_CAMERA_DEFAULT_FOV;
	...
	BlendTime = 0.5f;
	BlendFunction = EModularCameraModeBlendFunction::EaseOut;
	BlendExponent = 4.0f;
	BlendAlpha = 1.0f;
	BlendWeight = 1.0f;
}

UModularCameraComponent* UModularCameraMode::GetModularCameraComponent() const
{
	return CastChecked<UModularCameraComponent>(GetOuter());
}

UWorld* UModularCameraMode::GetWorld() const
{
...
{
	const UModularCameraComponent* ModularCameraComponent = GetModularCameraComponent();

	return ModularCameraComponent->GetTargetActor();
}
...
void UModularCameraMode::DrawDebug(UCanvas* Canvas) const
{
	check(Canvas);
	...
	DisplayDebugManager.DrawString(FString::Printf(TEXT("      ModularCameraMode: %s (%f)"), *GetName(), BlendWeight));
}
...
//////////////////////////////////////////////////////////////////////////
UModularCameraModeStack::UModularCameraModeStack()
{
	bIsActive = true;
}

void UModularCameraModeStack::ActivateStack()
{
	if (!bIsActive)
	{
		bIsActive = true;
		...
		for (UModularCameraMode* CameraMode : CameraModeStack)
		{
			check(CameraMode);
...
void UModularCameraModeStack::DeactivateStack()
{
	if (bIsActive)
	{
		bIsActive = false;
...
void UModularCameraModeStack::PushCameraMode(TSubclassOf<UModularCameraMode> CameraModeClass)
{
	if (!CameraModeClass)
	{
		return;
...
bool UModularCameraModeStack::EvaluateStack(float DeltaTime, FModularCameraModeView& OutCameraModeView)
{
	if (!bIsActive)
	{
		return false;
...
UModularCameraMode* UModularCameraModeStack::GetCameraModeInstance(TSubclassOf<UModularCameraMode> CameraModeClass)
{
	check(CameraModeClass);

	// First see if we already created one.
	for (UModularCameraMode* CameraMode : CameraModeInstances)
	{
	...
	UModularCameraMode* NewCameraMode = NewObject<UModularCameraMode>(GetOuter(), CameraModeClass, NAME_None, RF_NoFlags);
	check(NewCameraMode);
...
void UModularCameraModeStack::UpdateStack(float DeltaTime)
{
...
void UModularCameraModeStack::BlendStack(FModularCameraModeView& OutCameraModeView) const
{
	const int32 StackSize = CameraModeStack.Num();
	if (StackSize <= 0)
	{
...
void UModularCameraModeStack::DrawDebug(UCanvas* Canvas) const
{
	check(Canvas);
	...
	for (const UModularCameraMode* CameraMode : CameraModeStack)
	{
		check(CameraMode);
...
void UModularCameraModeStack::GetBlendInfo(float& OutWeightOfTopLayer, FGameplayTag& OutTagOfTopLayer) const
{
	...
	{
		UModularCameraMode* TopEntry = CameraModeStack.Last();
		check(TopEntry);
