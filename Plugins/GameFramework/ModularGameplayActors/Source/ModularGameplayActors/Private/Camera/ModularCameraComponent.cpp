...
void UModularCameraComponent::OnRegister()
{
	...
	if (!CameraModeStack)
	{
		CameraModeStack = NewObject<UModularCameraModeStack>(this);
...
void UModularCameraComponent::GetCameraView(float DeltaTime, FMinimalViewInfo& DesiredView)
{
	...
	FModularCameraModeView CameraModeView;
...
void UModularCameraComponent::UpdateCameraModes()
{
	...
	if (CameraModeStack->IsStackActivate())
	{
		...
		{
			if (const TSubclassOf<UModularCameraMode> CameraMode = DetermineCameraModeDelegate.Execute())
			{
...
void UModularCameraComponent::DrawDebug(UCanvas* Canvas) const
{
	check(Canvas);
	...
	DisplayDebugManager.DrawString(FString::Printf(TEXT("ModularCameraComponent: %s"), *GetNameSafe(GetTargetActor())));
...
void UModularCameraComponent::GetBlendInfo(float& OutWeightOfTopLayer, FGameplayTag& OutTagOfTopLayer) const
{
	check(CameraModeStack);
