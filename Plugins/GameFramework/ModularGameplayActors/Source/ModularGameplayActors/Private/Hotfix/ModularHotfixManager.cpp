...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ModularHotfixManager)

int32 UModularHotfixManager::GameHotfixCounter = 0;

UModularHotfixManager::UModularHotfixManager()
{
#if !UE_BUILD_SHIPPING
	OnScreenMessageHandle = FCoreDelegates::OnGetOnScreenMessages.AddUObject(this, &UModularHotfixManager::GetOnScreenMessages);
#endif // !UE_BUILD_SHIPPING

	HotfixCompleteDelegateHandle =
		AddOnHotfixCompleteDelegate_Handle(FOnHotfixCompleteDelegate::CreateUObject(this, &ThisClass::OnHotfixCompleted));
}

void UModularHotfixManager::Init()
{
	Super::Init();
}

void UModularHotfixManager::OnHotfixCompleted(EHotfixResult HotfixResult)
{
	...
	if (bHasPendingDeviceProfileHotfix)
	{
		...
		UGameSettingsLocal* GameSettings = UGameSettingsLocal::Get();
...
bool UModularHotfixManager::WantsHotfixProcessing(const FCloudFileHeader& FileHeader)
{
	bool bWantsProcessing = Super::WantsHotfixProcessing(FileHeader);
	if (!bWantsProcessing)
	{
		FString SupportedFiles[] = {TEXT("AssetMigrations.ini")};
...
bool UModularHotfixManager::HotfixIniFile(const FString& FileName, const FString& IniData)
{
	if (!bHasPendingDeviceProfileHotfix && FileName.EndsWith(TEXT("DEVICEPROFILES.INI"), ESearchCase::IgnoreCase))
	{
		...
		for (const auto& DPSection : AsConst(DeviceProfileHotfixConfig))
		{
			...
			if (DPSection.Key.Split(TEXT(" "), &DeviceProfileName, &DeviceProfileClass) &&
				DeviceProfileClass == *UDeviceProfile::StaticClass()->GetName())
			{
...
bool UModularHotfixManager::ApplyHotfixProcessing(const FCloudFileHeader& FileHeader)
{
...
bool UModularHotfixManager::ShouldWarnAboutMissingWhenPatchingFromIni(const FString& AssetPath) const
{
	return AssetPath.StartsWith(TEXT("/Engine/")) || AssetPath.StartsWith(TEXT("/Game/"));
}

void UModularHotfixManager::PatchAssetsFromIniFiles()
{
#if ENABLE_SHARED_MEMORY_TRACKER
...
void UModularHotfixManager::OnHotfixAvailablityCheck(
	const TArray<FCloudFileHeader>& PendingChangedFiles, const TArray<FCloudFileHeader>& PendingRemoveFiles)
{
...
#if !UE_BUILD_SHIPPING
void UModularHotfixManager::GetOnScreenMessages(TMultiMap<FCoreDelegates::EOnScreenMessageSeverity, FText>& OutMessages)
{
	// TODO Any messages/errors.
}
#endif // !UE_BUILD_SHIPPING

void UModularHotfixManager::RequestPatchAssetsFromIniFiles()
{
	if (!RequestPatchAssetsHandle.IsValid())
	{
		RequestPatchAssetsHandle = FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateWeakLambda(this,
...
void UModularHotfixManager::StartHotfixProcess()
{
#if UE_VERSION_5_8_x
	if (GIsEditor)
	{
