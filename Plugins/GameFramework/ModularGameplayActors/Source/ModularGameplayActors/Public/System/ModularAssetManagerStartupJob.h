...
struct FModularAssetManagerStartupJob
{
	...
	FModularAssetManagerStartupJob(const FString& InJobName, const TFunction<void(const FModularAssetManagerStartupJob&, TSharedPtr<FStreamableHandle>&)>& InJobFunc, float InJobWeight)
		: JobFunc(InJobFunc)
		, JobName(InJobName)
