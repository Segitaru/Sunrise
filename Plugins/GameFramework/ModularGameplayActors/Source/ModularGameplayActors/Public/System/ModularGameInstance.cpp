...
namespace Modular
{
	...
	static FAutoConsoleVariableRef CVarModularTestEncryption(TEXT("Modular.TestEncryption"), bTestEncryption,
		TEXT(
	...
	static FAutoConsoleVariableRef CVarModularUseDTLSEncryption(TEXT("Modular.UseDTLSEncryption"), bUseDTLSEncryption,
		TEXT("Set to true if using Modular.TestEncryption and the DTLS packet handler."), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarModularTestDTLSFingerprint(TEXT("Modular.TestDTLSFingerprint"), bTestDTLSFingerprint,
		TEXT(
	...
	static FAutoConsoleCommandWithWorldAndArgs CmdGenerateDTLSCertificate(TEXT("GenerateDTLSCertificate"),
		...
		FConsoleCommandWithWorldAndArgsDelegate::CreateLambda(
			[](const TArray<FString>& InArgs, UWorld* InWorld)
			{
				if (InArgs.Num() == 1)
				{
					...
					if (Cert.IsValid())
					{
						const FString CertPath = FPaths::ProjectContentDir() / TEXT("DTLS") /
												 FPaths::MakeValidFileName(FString::Printf(TEXT("%s.pem"), *CertName));
...
void UModularGameInstance::Init()
{
	...
	if (ensure(ComponentManager))
	{
		...
		ComponentManager->RegisterInitState(
			ModularGameplayTags::InitState_DataInitialized, false, ModularGameplayTags::InitState_DataAvailable);
		ComponentManager->RegisterInitState(
			ModularGameplayTags::InitState_GameplayReady, false, ModularGameplayTags::InitState_DataInitialized);
	}
...
AModularPlayerController* UModularGameInstance::GetPrimaryPlayerController() const
{
	return Cast<AModularPlayerController>(Super::GetPrimaryPlayerController(false));
}
...
void UModularGameInstance::HandlerUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error,
	ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	if (UGameLocalPlayer* const LocalPlayer = Cast<UGameLocalPlayer>(GetLocalPlayerByIndex(UserInfo->LocalPlayerIndex));
		IsValid(LocalPlayer))
	{
...
void UModularGameInstance::ReceivedNetworkEncryptionToken(const FString& EncryptionToken, const FOnEncryptionKeyResponse& Delegate)
{
...
#if UE_VERSION_5_8_x
#if UE_WITH_DTLS
		if (Modular::bUseDTLSEncryption)
		{
			TSharedPtr<FDTLSCertificate> Cert;

			if (Modular::bTestDTLSFingerprint)
			{
			...
			else
			{
				...
				const FString CertPath = FPaths::ProjectContentDir() / TEXT("DTLS") / TEXT("ModularTest.pem");
			...
			if (Cert.IsValid())
			{
				if (Modular::bTestDTLSFingerprint)
				{
					...
					FString DebugFile =
						FPaths::Combine(*FPaths::ProjectSavedDir(), TEXT("DTLS")) / FPaths::MakeValidFileName(EncryptionToken) +
						TEXT("_server.txt");
...
void UModularGameInstance::ReceivedNetworkEncryptionAck(const FOnEncryptionKeyResponse& Delegate)
{
	// This is a simple implementation to demonstrate using encryption for game traffic using a hardcoded key.
	// For a complete implementation, you would likely want to retrieve the encryption key from a secure source,
	// such as from a web service over HTTPS. This could be done in this function, even asynchronously - just
	...
	if (Modular::bUseDTLSEncryption)
	{
		...
		if (PlayerController && PlayerController->PlayerState && PlayerController->PlayerState->GetUniqueId().IsValid())
		{
			...
			if (Modular::bTestDTLSFingerprint)
			{
				...
				FString DebugFile = FPaths::Combine(*FPaths::ProjectSavedDir(), TEXT("DTLS")) / FPaths::MakeValidFileName(EncryptionToken) +
									TEXT("_server.txt");
