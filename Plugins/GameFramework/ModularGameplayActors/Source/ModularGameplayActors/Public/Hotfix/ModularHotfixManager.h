...
#include "ModularHotfixManager.generated.h"

UCLASS()
class UModularHotfixManager : public UOnlineHotfixManager
{
	GENERATED_BODY()
...
public:
	...
	UModularHotfixManager();
	virtual ~UModularHotfixManager();
...
protected:
	...
	virtual FString GetCachedDirectory() override { return FPaths::ProjectPersistentDownloadDir() / TEXT("Hotfix/"); }
	...
	virtual void OnHotfixAvailablityCheck(
		const TArray<FCloudFileHeader>& PendingChangedFiles, const TArray<FCloudFileHeader>& PendingRemoveFiles) override;
