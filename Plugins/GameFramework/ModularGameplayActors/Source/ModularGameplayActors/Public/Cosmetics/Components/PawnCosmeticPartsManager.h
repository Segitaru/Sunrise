...
#include "PawnCosmeticPartsManager.generated.h"
...
USTRUCT()
struct FGamePawnControllerCharacterPartEntry
{
	GENERATED_BODY()

	FGamePawnControllerCharacterPartEntry()
	{}
...
public:
	...
	FPawnCosmeticPartHandle Handle;
...
class UPawnCosmeticPartsManager : public UControllerComponent
{
	GENERATED_BODY()

public:
	UPawnCosmeticPartsManager(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	void AddCharacterPart(const FPawnCosmeticPart& NewPart);
	...
	UFUNCTION(BlueprintCallable, BlueprintAuthorityOnly, Category=Cosmetics)
	void RemoveCharacterPart(const FPawnCosmeticPart& PartToRemove);
...
protected:
	...
	TArray<FGamePawnControllerCharacterPartEntry> CharacterParts;

private:
	UPawnCosmeticCreatorComponent* GetPawnCustomizer() const;

	UFUNCTION()
	void OnPossessedPawnChanged(APawn* OldPawn, APawn* NewPawn);

	void AddCharacterPartInternal(const FPawnCosmeticPart& NewPart, ECharacterPartSource Source);

	void AddCheatPart(const FPawnCosmeticPart& NewPart, bool bSuppressNaturalParts);
	void ClearCheatParts();

	void SetSuppressionOnNaturalParts(bool bSuppressed);

	friend class UGamePawnCosmeticCheats;
};
